...
const StepLanguages = ({ userId, onBack, onFinish }: Props) => {
  ...
  const validate = (): boolean => {
    ...
    for (let i = 0; i < rows.length; i++) {
      if (!rows[i].name.trim()) {
        setError(`Language ${i + 1}: Name is required.`);
        return false;
      }
      if (!rows[i].proficiency) {
        setError(`Language ${i + 1}: Proficiency level is required.`);
        return false;
      }
    }
    setError(null);
        ...
        {rows.map((row, idx) => (
          <div key={idx} className="flex items-end gap-3">
            ...
            <button
              ...
              className="mb-1 shrink-0 text-muted-foreground hover:text-destructive transition-colors"
            >
