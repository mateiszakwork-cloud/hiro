...
interface EduBlock {
  ...
  fieldOfStudy: string;
  startYear: string;
  endYear: string; // number string or "expected"
}
...
const StepEducation = ({ userId, onNext, onBack }: Props) => {
  ...
  const validate = (): boolean => {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (!b.fieldOfStudy.trim()) {
        setError(`Education ${i + 1}: Field of study is required.`);
        return false;
      }
    }
    setError(null);
          ...
          <div key={idx} className="rounded-lg border border-border p-5 space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) => updateBlock(idx, { institution: e.target.value })}
                placeholder="e.g. Rotterdam School of Management"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Input
                  value={block.degree}
                  onChange={(e) => updateBlock(idx, { degree: e.target.value })}
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => updateBlock(idx, { fieldOfStudy: e.target.value })}
                  placeholder="e.g. Management in International Context"
                  className="rounded-lg"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select value={block.startYear} onValueChange={(v) => updateBlock(idx, { startYear: v })}>
                  ...
                  <SelectContent>
                    {START_YEARS.map((y) => <SelectItem key={y} value={String(y)}>{y}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                ...
                <Select value={block.endYear} onValueChange={(v) => updateBlock(idx, { endYear: v })}>
                  <SelectTrigger className="rounded-lg"><SelectValue placeholder="Year" /></SelectTrigger>
                  <SelectContent>
                    {END_YEARS.map((y) => <SelectItem key={y} value={String(y)}>{y}</SelectItem>)}
