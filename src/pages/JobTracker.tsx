...
const JobTracker = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-[28px] font-bold" style={{ color: "#0F1F3D" }}>
          Job Tracker
        </h1>
        <p className="text-muted-foreground mt-1">
          Paste a job URL below to automatically fill in all details.
        </p>
      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/40">
                    {["Company", "Job Title", "Function", "Location", "Work Mode", "Duration", "Status", "Match", "Added"].map(
                      (h) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <div
                            className="h-8 w-8 rounded bg-muted flex items-center justify-center text-xs font-bold text-muted-foreground shrink-0"
                          >
                      ...
                      <td className="px-4 py-3">
                        {job.function ? (
                          <span
                            className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium ${FUNCTION_COLORS[job.function] || FUNCTION_COLORS.Other}`}
                          >
                            {job.function}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3">
                        {job.work_mode ? (
                          <span
                            className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium ${WORK_MODE_COLORS[job.work_mode] || WORK_MODE_COLORS.Onsite}`}
                          >
                            {job.work_mode}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        ...
                        >
                          <SelectTrigger
                            className={`h-7 w-auto border-0 gap-1 px-2.5 rounded-full text-xs font-medium ${getStatusColor(job.status)}`}
                          >
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <td className="px-4 py-3">
                        {job.match_score !== null ? (
                          <span
                            className={`inline-flex items-center justify-center h-8 w-8 rounded-full border text-xs font-bold ${getScoreColor(job.match_score)}`}
                          >
                            {job.match_score}
                          </span>
                        ) : (
          ...
          {selectedJob && (
            <div className="p-6">
              <SheetHeader className="mb-6">
                <SheetTitle className="text-xl" style={{ color: "#0F1F3D" }}>
